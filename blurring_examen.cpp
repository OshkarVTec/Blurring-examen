...
int main(){
   ...
   for (int i = 0; i < 1; i++){
      for (int j = 4; j <= 7; j+=3) { //Se indica el tamaño de la matriz de blurring. j = (tamaño-1)/2
         blur(i+1,j);
      }
      for (int j = 11; j <= 20; j+=3) { //Se indica el tamaño de la matriz de blurring. j = (tamaño-1)/2
         blur(i+1,j);
      }
   }
